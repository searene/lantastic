...
import {Segment, Input, Icon, Table, Menu, Label} from 'semantic-ui-react';
...
class ConnectedCardBrowser extends React.Component<CardBrowserProps, CardBrowserStates> {
  ...
  private TABLE_DATE_FORMAT = 'YYYY-MM-DD HH:mm:ss';
    ...
    return (
      <Segment className={"card-browser-segment"}>
        ...
        <div className={"table-row"}>
          {this.state.cards === undefined ? '' : this.getTable()}
        </div>
      </Segment>
    )
  }
        ...
        <Table.Body>
          {this.state.cards.map(card => (
            <Table.Row key={card[CARD_COLUMN_ID]}>
              ...
              <Table.Cell>{moment(card[CARD_COLUMN_CREATION_TIME], DATE_FORMAT).format(this.TABLE_DATE_FORMAT)}</Table.Cell>
              <Table.Cell>{moment(card[CARD_COLUMN_NEXT_REVIEW_TIME], DATE_FORMAT).format(this.TABLE_DATE_FORMAT)}</Table.Cell>
            </Table.Row>
          ))}
        </Table.Body>
  ...
  private getPageLimitStatement = (): string => {
    return `LIMIT ${this.PAGE_SIZE * (this.state.currentPage - 1)}, ${this.PAGE_SIZE * this.state.currentPage}`;
  };
  private getTotalPageCount = async (): Promise<number> => {
    const db = await Sqlite.getDb();
    const result = await db.get(`
                    SELECT COUNT(*) AS count
                    FROM ${CARD_TABLE}
    `);
    return Math.ceil(result.count / this.PAGE_SIZE);
  }
}
